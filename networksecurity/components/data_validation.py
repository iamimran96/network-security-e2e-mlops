...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"Required num3ber of columns: {number_of_columns}")
    ...
    def detect_data_drift(self, base_df, current_df, threshold=0.05) -> bool:
        try:
            ...
            for column in base_df.columns:
                ...
                is_simple_dist = ks_2samp(d1, d2)
                if threshold <= is_simple_dist.pvalue:
                    is_found = False
                else:
                    is_found = True
                    status = False
                report.update({column: {"pvalue": float(is_simple_dist.pvalue), "drift_status": is_found}})
